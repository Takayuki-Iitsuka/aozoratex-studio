...
_COMPILE_LOGS: dict[str, dict[str, object]] = {}
_COMPILE_LOG_MAX_LINES = 2500
_COMPILE_LOG_TTL_SECONDS = 60 * 60
...
def _cleanup_stale_compile_logs(now: Optional[float] = None) -> None:
    current = now if now is not None else time.time()
    stale_client_ids = [
        client_id
        for client_id, entry in _COMPILE_LOGS.items()
        if current - float(entry.get("updated_at", current)) > _COMPILE_LOG_TTL_SECONDS
    ]
    for client_id in stale_client_ids:
        _COMPILE_LOGS.pop(client_id, None)


def _reset_compile_logs(client_id: Optional[str]) -> None:
    client = str(client_id or "").strip()
    if not client:
        return

    with _COMPILE_LOG_LOCK:
        _cleanup_stale_compile_logs()
        _COMPILE_LOGS[client] = {
            "lines": [],
            "last_seq": 0,
...
def _append_compile_log(client_id: Optional[str], line: str) -> None:
    client = str(client_id or "").strip()
    if not client or not line:
        return

    with _COMPILE_LOG_LOCK:
        _cleanup_stale_compile_logs()
        entry = _COMPILE_LOGS.setdefault(
            client,
            {"lines": [], "last_seq": 0, "updated_at": time.time()},
        )
        last_seq = int(entry.get("last_seq", 0)) + 1
        lines = list(entry.get("lines", []))
        lines.append({"seq": last_seq, "line": str(line)})
        if len(lines) > _COMPILE_LOG_MAX_LINES:
            lines = lines[-_COMPILE_LOG_MAX_LINES:]
...
def _read_compile_logs(client_id: Optional[str], after: int = 0) -> tuple[list[dict], int]:
    client = str(client_id or "").strip()
    if not client:
        return [], 0
    ...
    with _COMPILE_LOG_LOCK:
        _cleanup_stale_compile_logs()
        entry = _COMPILE_LOGS.get(client)
        if not entry:
            return [], 0
...
def _resolve_generation_defaults(
    ...
    return server_services.resolve_generation_defaults(device, bg_color, fg_color)
...
def _list_background_assets() -> dict[str, object]:
    return server_services.list_background_assets()
...
@app.route("/api/background-assets")
def api_background_assets():
    """背景画像候補（表紙・和紙）の一覧を返す。"""
    return jsonify({"success": True, **_list_background_assets()})
...
@app.route("/api/compile-logs")
def api_compile_logs():
    ...
    lines, last_seq = _read_compile_logs(client_id, after=after)
    return jsonify(
        {
            "success": True,
            "client_id": client_id,
...
def api_generate():
    ...
    device, bg_color, fg_color = _resolve_generation_defaults(
        data.get("device"),
        data.get("bg_color"),
        data.get("fg_color"),
    )
    ...
    _reset_compile_logs(client_id)

    def handle_log(line: str):
        _append_compile_log(client_id, line)
...
def api_generate_batch():
    ...
    def run_one(source: str) -> tuple[bool, dict]:
        def handle_log(line: str):
            _append_compile_log(client_id, f"[{Path(source).name}] {line}")
...
def serve_colors_html():
    return redirect("/static/docs/manuals/colors-visualizer.html", code=301)
...
def serve_device_paper_size_map_html():
    return _serve_html_file_with_theme(
        DOCS_DIR / "manuals" / "device-paper-size-map.html"
    )
